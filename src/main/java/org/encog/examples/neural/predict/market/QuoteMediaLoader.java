...
/**
 * This class loads financial data from Quotemedia.
 * 
 * @author ahernandez
 */
public class QuoteMediaLoader implements MarketLoader {

	/**
	 * This method builds a URL to load data from QuoteMedia Finance for a neural
	...
	private URL buildURL(final TickerSymbol ticker, final Date from,
		...
		form.add("symbol", ticker.getSymbol().toUpperCase());
		form.add("startMonth", "" + calendarFrom.get(Calendar.MONTH));
		form.add("startDay", "" + calendarFrom.get(Calendar.DAY_OF_MONTH));
		form.add("startYear", "" + calendarFrom.get(Calendar.YEAR));
		form.add("endMonth", "" + calendarTo.get(Calendar.MONTH));
		form.add("endDay", "" + calendarTo.get(Calendar.DAY_OF_MONTH));
		form.add("endYear", "" + calendarTo.get(Calendar.YEAR));
		form.add("isRanged", "true");
		os.close();
		final String str = "http://app.quotemedia.com/quotetools/getHistoryDownload.csv?&webmasterId=501&"
				+ os.toString();
		return new URL(str);
	}
	...
	public Collection<LoadedMarketData> load(final TickerSymbol ticker,
		...
		try {
			...
			while (csv.next()) {
				...
				final double adjClose = csv.getDouble("adjclose");
